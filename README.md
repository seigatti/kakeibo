...
- **ふるさと納税**（ふるさとタブ）: せ/あ・年を切り替えて上限額と寄付を管理。年収・社会保険料・医療費控除を入れると上限の目安を自動計算（税額通知書の正確な値があれば手動指定が優先）。楽天の商品ページで専用ブックマークレット（設定タブ）をクリックすると商品名・金額・自治体・URLが入力済みの追加画面が開く。「未購入」ステータス＝候補リストとして管理され、購入したら年とステータスを更新する
